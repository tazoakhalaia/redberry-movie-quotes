...
<div class="w-28 flex justify-center items-center">
    <div class="ml-2">
        <a href="{{ route('movies', ['movie' => $movie->id, 'lang' => 'en']) }}"><button class="rounded-full text-white border border-slate-100 p-2 hover:bg-white hover:text-black">{{ trans("profile.en") }}</button></a>
        <a href="{{ route('movies', ['movie' => $movie->id, 'lang' => 'ka'])}}"><button class="block rounded-full text-white border p-2 border-slate-100 hover:bg-white hover:text-black mt-2">{{ trans("profile.ka") }}</button></a>
    </div>
</div>
